...
import { getProjects, getProjectTasks, createProject, createTask, getProjectById } from '../api';
...
export default function Home() {
    ...
    const fetchMembers = async () => {
      try {
        ...
        if (owner) {
          ...
          allPeople.push(typeof owner === 'object' ? { ...owner, _roleLabel: 'Owner' } : { _id: owner, _roleLabel: 'Owner' });
        }
  ...
  const handleProjectClick = (projectId) => {
    const id = typeof projectId === 'object' && projectId !== null
  ...
  const handleCreateProject = async (e) => {
    ...
    try {
      ...
      setProjectFormError(err.response?.data?.message || 'Failed to create project');
    } finally {
  ...
  const handleCreateTask = async (e) => {
    ...
    try {
      ...
      const payload = { title: taskForm.title.trim(), dueDate: taskForm.dueDate, assignee: taskForm.assignee };
      if (taskForm.description.trim()) payload.description = taskForm.description.trim();
      ...
      {showProjectForm && (
        <div className='inline-form'>
          <form onSubmit={handleCreateProject}>
            {projectFormError && <div className='alert alert-error'>{projectFormError}</div>}
      ...
      {loading ? (
        ...
        <>
          ...
          {projects.length === 0 && !showProjectForm ? (
            <p className='empty-state'>No projects yet. Click "+ New Project" to create one.</p>
          ) : (
          ...
          {selectedProject && (
            <div className='tasks-section'>
                      ...
                      <div className='form-group'>
                        ...
                        >
                          ...
                          {projectMembers.map((m) => {
                            ...
                            const name = typeof m === 'object' ? (m.userName || m.email || id) : id;
                            const label = m._roleLabel ? `${name} (${m._roleLabel})` : name;
              ...
              {tasksLoading ? (
                ...
                <p className='empty-state'>No tasks in this project. Click "+ New Task" to add one.</p>
              ) : (
